 ...
 * 文件名：   FuJinDataConverter
 * 创建者：   DoDo
 * 创建时间： 2017/10/16 0:25
 * 描述：     TODO
 */

public class FuJinDataConverter extends DataConverter {
    ...
    public ArrayList<MulEntity> convert() {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            final String region = auther.getString("region");
            ...
            final FuJinBean fuJinBean = new FuJinBean();
            fuJinBean.setHeadImg(headImg);
            fuJinBean.setHeadName(headName);
            fuJinBean.setCover(cover);
            fuJinBean.setRegion(region);

            final MulEntity entity = MulEntity.builder()
                    .setItemType(ItemType.CONTENT_3)
                    .setBean(fuJinBean)
                    .build();
            ENTITIES.add(entity);
